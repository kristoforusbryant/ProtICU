...
class protop_loss(): 
    ...
    def __call__(self, out, min_dis, y, protop_classes, verbose=False): 
        loss =  self.ce(out, y.reshape(-1).long())\
                + cluster_loss(min_dis, y, protop_classes) \
                + separation_loss(min_dis, y , protop_classes)
        print(self.ce(out, y.reshape(-1).long()))
        print(cluster_loss(min_dis, y, protop_classes))
        print(separation_loss(min_dis, y , protop_classes))
        return loss
